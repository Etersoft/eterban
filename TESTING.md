...
> Внимание: сначала выполнять проверки только на выделенной тестовой ВМ.
> Команды ниже с реальными IP, `priv`, `sprintbox` и `hetzner` являются
> production-runbook и требуют отдельного явного согласования; не использовать
> их как обычный smoke-test разработки или сборки пакета.

## Быстрый smoke-test на тестовой ВМ

```bash
sh tests/static-checks.sh
```

После неё выполнить clean RPM build в подготовленном test topdir; не запускать
команды из последующих разделов против production без отдельного согласования.
